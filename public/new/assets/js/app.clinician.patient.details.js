...
function exploder(params) {
    ...
    //     $('#' + params).closest("tr").next("tr").children("td").slideUp(100);
    //     $('#' + params).closest('tr').addClass('active')
    // }
    // else {
    //     $('#' + params).closest("tr").next("tr").children("td").slideDown(100);
    //     $('#' + params).closest('tr').prev().children('td').slideUp(100);
    //     $('#' + params).closest("tr").prev("tr").toggleClass("d-none");
    // }
    ...
    $('#' + params).closest("tr").next("tr").toggleClass("d-none");
    $('#' + params).find('.la-plus').removeClass('la-plus').addClass('la-minus');
    if ($('#' + params).closest('tr').next('tr').not('.d-block')) {
        $('#' + params).closest("tr").next("tr").children("td").slideDown(50);
    }
    if ($('#' + params).closest('tr').next('tr').hasClass('d-none')) {
        $('#' + params).find('.la-minus').removeClass('la-minus').addClass('la-plus');
    }
    if ($('#' + params).closest('tr').next('tr').not('.d-block')) {
        $('#' + params).last().closest("tr").prev('tr').children("td").slideUp(50);
    }
}
...
$('.addInsurance').on('click', function () {
    $('#v-pills-tabContent a[href="#insurance"]').tab('show')
})
