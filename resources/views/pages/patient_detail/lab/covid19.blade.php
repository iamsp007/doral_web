                    ...
                    <tbody>
                        ...
                        @if(count($covids) > 0)
                            @foreach($covids as $covid)
                                <x-hidden name="patient_lab_report_id" id="patient_lab_report_id" value="{{ $covid->id }}" />
                                <tr class="drug-main-tr">
                                    <td scope="row">{{ $number }}</td>
                                    <td scope="row">{{ ($covid->labReportType) ? $covid->labReportType->name : ''}}</th>
                                    <td>{{ $covid->due_date }}</td>
                                    <td>{{ ($covid->perform_date) ? $covid->perform_date : 'N/A' }}</td>
                                    <td>{{ $covid->expiry_date }}</td>
                                    <td>{{ $covid->result }}</td>
                                    @role('clinician')
                                        <td class='text-center'>
                                            <input type="file" class="uploadLabResult" id="{{ $covid->labReportType->id }}" data-id="{{ $covid->labReportType->id }}" ></input>
                                        </td>
                                    @endrole
                                </tr>
                                @php $number++; @endphp
                        ...
                        @role('clinician')
                            <tr>
                                ...
                                <form id="drugScreenForm">
                                    @csrf
                                    <th scope="row" class="drug-sequence">{{ ($covids) ? $covids->count() + 1 : '' }}</th>
                                    <td>
