...
@php
$event = array();
foreach($response['datarow'] as $dt){
$event[] = array('title'=>'Appoitment Request ('.$dt->total.')','start'=>date('Y-m-d',strtotime($dt->start_datetime)),'end'=>date('Y-m-d',strtotime($dt->end_datetime)),'url'=>"#"); 
}
foreach($response['dataremindar'] as $dtr){
$time = explode('-',$dtr->start_end_time);
$starttime = trim($time[0]);
$endtime = trim($time[1]);
$event[] = array('title'=>'Remindar Request ('.$dtr->total.')','start'=>date('Y-m-d',strtotime($dtr->startdate)),'end'=>date('Y-m-d',strtotime($dtr->startdate)),'start'=>$dtr->startdate.'T'.$starttime,'end'=>$dtr->startdate.'T'.$endtime,'url'=>"remindar-calendar/".base64_encode($dtr->startdate).""); 
}
